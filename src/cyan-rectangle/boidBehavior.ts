...
export function updateBoids(
  ...
  boids.forEach(boid => {
    const distToPlayer = Math.sqrt(
      (boid.x - playerX) ** 2 + (boid.y - playerY) ** 2
    );
    ...
    let playerAvoidY = 0;
    if (distToPlayer < playerAvoidanceRadius) {
      const dx = boid.x - playerX;
      const dy = boid.y - playerY;
    ...
    for (const rock of rocks) {
      // Check distance to rock center
      const rockCenterX = rock.x + rock.width / 2;
      const rockCenterY = rock.y + rock.height / 2;
      const dx = boid.x - rockCenterX;
      const dy = boid.y - rockCenterY;
      const dist = Math.sqrt(dx * dx + dy * dy);
